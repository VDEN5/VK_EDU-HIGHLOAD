...
1) 49M MAU
2) 16,2M DAU
3) 20 000 000 писем отправляются в день
4) 37% респондентов проверяют почту несколько раз в сутки, у 22% она всегда открыта в течение дня. 20% - раз в день, 11% – раз в неделю. 7% - крайне редко, а 3% - очень давно
5) 50 пб - хранилище mail.ru. Из них 15 пб - письма, 35 пб - вложения (файлы)
6) 8 гб - бесплатный размер хранилища писем mail.ru
7) Средний вес письма без вложения 75 кб
8) Максимальный размер вложений 25 мб
9) 100 млн активных пользователей
10) По данным специалистов Почты Mail.Ru около 50% всех писем в Мобильной Почте Mail.Ru отправляются с вложениями. В каждом пятом письме — два и более прикрепленных файла. Примерно в 80% случаях — это фотографии, 19% — документы, 1% приходится на видеоролики и архивы
...
| Хранилище | 0,5 гб/пользователь | 50 млн MAU - количество хранилищ. Пусть ящики неактивных пользователей удаляются. 50 пб / 100 млн пользователей = 0,52 гб |
| Проверка почты | 3.5 раз/день | Пусть 37% — проверяют почту 3 раза в сутки, 22% — 10 раз, 20% — 1 раз, 11% — 0.14, 7% — 0.01, 3% — 0. <br> Тогда среднее число проверок в день = (0.37×3) + (0.22×10) + (0.20×1) + (0.11×0.14) + (0.07×0.01) + (0.03×0) = 3.5261 |
| Пользователи | 100 млн | В источнике указаны активные пользователи. Пусть неактивные пользователи удаляются, тогда можно считать это значение как количество пользователей |
