...
| Сервис | Назначение | Базовый RPS | Пиковый RPS (×2) |
|--------|------------|-------------|------------------|
| DeliveryService | Обработка отправки писем | 231 | 462 |
| MailService | Просмотр писем, работа с черновиками | 8854 + 266 = 9120 | 18 240 |
| UserService | Управление профилями пользователей | 295 + 578 = 873 | 1 746 |
| AuthService | Аутентификация и авторизация | 295 | 590 |
| FileService | Работа с вложениями | 26 562 | 53 124 |

**Расчет ресурсов для микросервисов:**

| Сервис | RPS | Пиковый RPS | CPU (ядер) | RAM (GB) | Обоснование |
|--------|-----|-------------|------------|----------|-------------|
| DeliveryService | 231 | 462 | 4 | 8 | Лёгкая нагрузка: 1 ядро = ~100 RPS (Go) |
| MailService | 9120 | 18 240 | 91 | 91 | Средняя нагрузка: 1 ядро = ~100 RPS (Go) |
| UserService | 873 | 1 746 | 18 | 18 | Средняя нагрузка |
| AuthService | 295 | 590 | 6 | 6 | Лёгкая нагрузка |
| FileService | 26 562 | 53 124 | 266 | 266 | Высокая нагрузка: 1 ядро = ~100 RPS |

**Инфраструктурные компоненты:**

| Компонент | Назначение | RPS | Пиковый RPS | CPU (ядер) | RAM (GB) | Обоснование |
|-----------|------------|-----|-------------|------------|----------|-------------|
| Nginx L4 | Балансировка SMTP/IMAP трафика | ~30 000 | 60 000 | 120 | 120 | 1 ядро = 500 CPS |
| Nginx L7 | Балансировка HTTP/HTTPS трафика | ~30 000 | 60 000 | 120 | 120 | 1 ядро = 500 RPS |
| RabbitMQ | Очереди сообщений | 231 | 462 | 2 | 16 | Лёгкая нагрузка |
| Redis | Кэширование, сессии | 295 + 8854 = 9149 | 18 298 | 92 | 32 | Высокая нагрузка на кэш |
| Prometheus | Мониторинг и метрики | ~60 000 | 120 000 | 8 | 64 | Сбор метрик со всей системы |

**Системы хранения данных:**

| Хранилище | Назначение | RPS | Пиковый RPS | CPU (ядер) | Объем | Обоснование |
|-----------|------------|-----|-------------|------------|-------|-------------|
| Cassandra | Хранение писем и метаданных | 9120 + 231 + 266 = 9617 | 19 234 | 96 | 36 ТБ | Высокая нагрузка на чтение/запись |
| S3 mail storage | Хранение файлов-вложений | 26 562 | 53 124 | 266 | 35 ПБ | Очень высокая нагрузка на файлы |
| PostgreSQL | Пользователи, папки, системные данные | 873 + 590 = 1463 | 3 000 | 30 | ~210 ГБ | Транзакционные данные с репликацией |

### Хостинг-провайдеры

Выбрана гибридная модель развертывания с использованием оркестрации в Kubernetes. Из-за большого объема данных Cassandra и S3 логичнее развертывать на собственном железе.

**Собственная инфраструктура (on-premise):**

| Компонент | Конфигурация | Cores | Количество | Стоимость покупки |
|-----------|--------------|-------|------------|-------------------|
| Storage Node (Cassandra) | Xeon E5-2690/2x 64GB/24x30TB 2.5 NVMe | 96 | 8 | 1.3M$ |
| Storage Node (S3) | Xeon E5-2690/2x 128GB/24x30TB 2.5 NVMe | 266 | 17 | 2.9M$ |
| Nginx балансировщики | AMD EPYC 7313P 32GB 250GB | 240 | 15 | 39 420$ |
| Application Nodes | AMD EPYC 7313P 64GB 500GB | 385 | 24 | 63 072$ |

**Облачная инфраструктура (VK Cloud):**

| Компонент | Конфигурация | Cores | Количество | Стоимость аренды |
|-----------|--------------|-------|------------|------------------|
| k8s-cluster | 24x(2CPU/32GB/100GB SSD) + Load Balancer | 192 | 24 | 384 000₽/мес |

**Итоговые ресурсы по дата-центрам:**

С учетом распределения нагрузки между 5 дата-центрами, ресурсы делятся пропорционально:

| Ресурс | Всего | На один ДЦ |
|--------|-------|------------|
| CPU для микросервисов | 385 ядер | ~77 ядер |
| RAM для микросервисов | 389 GB | ~78 GB |
| CPU для Cassandra | 96 ядер | 19 ядер |
| CPU для S3 Storage | 266 ядер | 53 ядер |
| Хранилище Cassandra | 36 ТБ | 7.2 ТБ |
| Хранилище S3 | 35 ПБ | 7 ПБ |
